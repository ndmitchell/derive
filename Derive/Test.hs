...
overlap = ["BinaryDefer","EnumCyclic","LazySet"]

exclude = ["ArbitraryOld","PlateTypeable","Ref","Serial"]

-- These must come first if they are present, since Ord/Data depend on them
...
test = do
    ...
    srcs <- sequence [readSrc $ "Data/Derive" </> derivationName d <.> "hs" | d <- order derivations]
    let dev = filter (flip notElem exclude . derivationName . snd) $ zip srcs (order derivations)
    
    let (dev1,dev2) = partition (flip notElem overlap . derivationName . snd) dev
    autoTest (name++"1") decls dev1
    autoTest (name++"2") decls dev2
...
autoTest :: String -> [DataDecl] -> [(Src,Derivation)] -> IO ()
autoTest name ts ds =
    writeFile (name++".hs") $ unlines $
        ...
        [prettyPrint i | (s,_) <- ds, i <- srcImportStd s] ++
        [prettyPrint t | t <- ts2] ++
        ["$(derives [make" ++ derivationName d ++ "] " ++ types ++ ")" | (_,d) <- ds]
    where
...
order ds = yes ++ no
    where (yes,no) = partition (flip elem priority . derivationName) ds
