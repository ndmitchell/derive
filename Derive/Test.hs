...
test = do
    ...
    writeFile name $ unlines $ autoTest types derivations
...
autoTest :: [DataDecl] -> [Derivation] -> [String]
autoTest ts ds =
    ["{-# LANGUAGE TemplateHaskell #-}"
    ,"import Data.DeriveTH"
    ,"main = putStrLn \"Type checking successful\""] ++
    [prettyPrint t | t <- ts, dataDeclName t `notElem` ["Bool","Either"]] ++
    ["$(derive make" ++ derivationName d ++ " ''" ++ dataDeclName t ++ ")" | t <- ts, d <- ds]
