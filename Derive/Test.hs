...
test = do
    ...
    writeFile name $ unlines $ autoTest srcs types derivations
...
autoTest ss ts ds =
    ...
    [prettyPrint t | t <- ts2] ++
    ["$(derives [make" ++ derivationName d ++ "] [" ++ intercalate "," ["''" ++ dataDeclName t | t <- ts2] ++ "])" | d <- ds2]
    where
        ts2 = filter (flip notElem ["Bool","Either"] . dataDeclName) ts
