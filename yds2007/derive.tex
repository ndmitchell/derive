...
The examples shown previously map values to constant-sized code fragments. But consider the pattern |CtorTwo x1 x2|, we require |n| variables, where |n| is the arity of the constructor. To solve this problem, when dealing with a list of fragments, if (1) any item has a function which is is correct on all fragments, and (2) their parameter values are sequential, then a generalised map is constructed. In the above example:
...
The multiple ways that a number may be realised means that the generation of functions operates on a list of possible functions \cite{wadler:list_successes}. By using many possible functions, when applying the |MAP| rule, it is more likely that one of the functions will be applicable to all the elements of the list.
...
instance Eq \? <| dataname |> where
    <| MAP ctors (
        ...
        (<| ctorname |> \? <| MAP [1..ctorarity] (x <| i |> ) ) =
        <| FOLDR fields (&&) True \? <| MAP [1..i] (x <| i |> == y <| i |>) |> |>
    ) |>
    _ == _ = False
\end{code}
