...
In order to compose many hypothesises into a single hypothesis, it is necessary to parameterise them by more than just a data type. We allow a hypothesis to be parameterised by any of:

\paragraph{A data type} In the first example the whole code is parameterised by the data type. One particular place where the parameterisation by data type shows up is |Eq (DataName alpha)|. Here the text |DataName| is the name of the data type.

\paragraph{A constructor} The sample |CtorTwo x1 x2| is parameterised by the constructor |CtorTwo|. This parameterisation is responsible both for the name |CtorTwo|, and for the number of fields, namely two.
