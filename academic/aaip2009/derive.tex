...
Our DSL type is given in Figure \ref{fig:dsl}, and our |apply| function is given in Figure \ref{fig:apply}. The operations in the DSL are split into six groups -- we first give a high-level overview of the DSL, then return to each group in detail. The |apply| function is written in terms of |applyEnv|, where an environment is passed including the input data type, and optionally a constructor in scope. Some functions in the DSL add to the environment (i.e. |MapCtor|), while some read from the environment (i.e. |CtorName|). Any operation reading a value from the environment must be nested within an operation placing that value in the environment.
...
Some type classes require information on which module a type resides within, for example a |Typeable| instance \cite{lammel:syb} follows the pattern:
...
The |Typeable| class performs runtime type comparison, so for each distinct type needs a distinct string on which to compare, so the module name is used to disambiguate. Our |Input| type does not include information on the module name, so cannot be used to derive |Typeable|. It would be possible to assign the defined string |"Module.Name"| as the module of the input sample, and treat it similarly to the string |"Sample"|. However, the only instance we are aware of that requires the module name is |Typeable|. Additionally, the module name is a property of the whole file, while all other |Input| information can be obtained from just the data type definition -- this locality property seems too valuable to throw away for just a single additional type class.
...
This section describes our practical experiences with trying to specify instances in a form where they can be derived. Ideally we would like to write all instances in a natural way, but sometimes we need to make concessions to our derivation algorithm. Using the techniques given here, it seems to be possible to write most instances which are based on information included in the |Input| type.
...
When naming variables it is important that a series of related variables follow the same pattern, i.e. in the example from \S\ref{sec:fromCtorName} we used |Second x_1 x_2|, rather than |Second x y|. By using a naming scheme based on sequential numbers we are able to derive the fields correctly.

\subsubsection{Explicit Fold Base-Case}

When performing a fold, it is import to always explicitly include the base-case. For example, in the introductory example of |NFData| the |Second| alternative is specified as |rnf x_1 `seq` rnf x_2 `seq` ()|, however we can show that:

\ignore\begin{code}
forall x `o` rnf x `seq` () == rnf x
\end{code}

Therefore we could write the |Second| alternative more compactly as |rnf x_1 `seq` rnf x_2|. However, doing so would mean that there was not one consistent pattern that could be applied for all constructors, and the derivation would fail. In general, when considering folds, the base-case should always be written explicitly.
...
The pattern for the number of constructors is useful and simple to implement, but seems a little verbose. In the first version of \derive{} the constructor count was guessed from the number 3. Unfortunately, the inclusion of this guess breaks the restrictions we have imposed for predictability. Another way of simplifying this pattern would be to introduce a meta function |ctorCount|, which expanded to the number of constructors. This solution means examples are no longer real example instances, and requires people to learn some of the DSL, something we have tried to avoid. In the end, we simply accept that the constructor count is slightly verbose.

\subsection{Timing Properties}

We have implemented the methods described in this paper, and have used them to guess all 15 examples referred to in \S\ref{sec:failure}, along with 2 additional test cases. For each example we have performed the following steps:

\begin{enumerate}
\item We derive the DSL from an example.
\item We apply the DSL (without output optimisation) to the |Sample| data type and check it matches the input.
...
To perform all steps for 17 examples took 0.3 seconds when compiled with GHC at no optimisation on a laptop with a 2GHz CPU and 1Gb of RAM. We consider these times to be more than adequate, so have not carried out further experiments or investigated further optimisations.

\section{Related Work}
\label{sec:related}

\todo{write a decent related work section}

The first version of the \derive{} tool was described in a previous paper \cite{me:yds_derive}. The original work set out to achieve the same goals, and had much the same success, but it lacked a number of critical notions. In the original work there was no explicit DSL, the derivation function was combined with the DSL. In particular the original work had no notion of predictability. The lists have also been inferred in a much better way.

We are unaware of any work (other than our own) that attempts to automatically derive Haskell type classes. Therefore we split the remaining related work in to two sections -- that which hopes to specify type classes and that which hopes to derive relationships.

\subsection{Specifying Type Classes}

There have been other attempts at writing derivable type classes. The tool with a similar spirit to \derive{} is DrIFT \cite{drift}. In constrast to our tool, there is no inference of type classes and no underlying DSL. However, to a users point of view, the tools are similar. However, because we derive examples easily it has been possible to dramatically increase the number of classes at speed -- even thought DrIFT is old, Derive supports more classes.

The Deriveable Type Classes work \cite{hinze:derivable_type_classes} allows a user to write type class instances explicitly in terms of units, products and sums, which combine to form all data types. There are many restrictions on these types, for example record definitions aren't allowed and derivations depending on the types aren't allowed. In fact, we suspect that most instances that can be expressed in their scheme can also be derived by example.

\subsection{Deriving Relationships}

The purpose of this work is to find a pattern, and generalise that pattern to other situations. Genetic algorithms \cite{genetic_algorithms} are often used to automatically find a pattern in a data set. Genetic algorithms work by evolving a hypothesis (a gene sequence) and testing on a sample problem. They are well suited to search problems where the utility function is continuous -- close hypotheses have similar fitness. The main difference from this paper is that these tools are random search, while ours is precise induction.

Some work takes non-recursive equations and generalising them into recursive equations \cite{kitzelmann:inductive_synthesis,kitzelmann:data_driven_induction}, working from a program specification of possibly non-ground equations. Other work uses only ground examples \cite{kitzelmann:data_driven_induction_ground}. All these pieces of work use a set of only positive examples.

Some work performs exhaustive search \cite{katayama:exhaustive_generation}, which is a cool technique and could perhaps be used within our work as the search mechanism.

The big difference is that we start from only one example, and we restrict the problem domain in such a way that any answer we get is correct.

The closest work we are aware of is that of the theorem proving community. Induction is a very common tactic for writing proofs, and well supported in systems such as HOL Light \cite{hol_light}. Typically the user must suggest the use of induction, which the system checks for validity. Automatic inference of an induction argument has been tried \cite{mintchev:reasoning}, but is rarely successful. However, these systems all work from one positive example, attempting to determine a reasonably restricted form of example.
...
\item While our program is written in the typed language Haskell, most of the actual DSL operations are implemented upon a universal data type with dynamic type conversions -- essentially a dynamically typed language. In order to preserve types throughout we could make use of as GADTs \cite{spj:gadt}.
\item We have implemented our scheme specifically for instance generators in Haskell, but the same scheme could be applied to other computer languages and other situations. One possible target would be F\#, where there are interfaces instead of type classes. Another target could be the object-orientated community, who have embraced design patterns \cite{design_patterns}, which often involve recurring patterns parameterised by data type.
\end{itemize}
...
Thanks to Stefan O'Rear help with the first version of the \derive{} tool. Thanks to Niklas Broberg for the excellent haskell-src-exts library. Thanks to Hongseok Yang for fruitful discussions on the original instance generation work.
