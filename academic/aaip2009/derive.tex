...
alpha,beta,d_1,d_2,Q,Append,i,f,x_1,x_2,x,y,m,z,d,op,a,b,xs,ys
dataArity,DataArity
\end{comment}}
\begin{comment}
...
Operations have restrictions on what the types arguments must evaluate to, and what environment values must be available. It would be possible to capture many of these invariants using either phantom types \cite{phantom_types} or GADTs \cite{spj:gadt}. However, for simplicity, we choose not to.
...
We present the output simplification directly as we have implemented it, using the Uniplate generics library \cite{me:uniplate}. All of the rules operate at the expression level, and each rule is correct individually. To easily express matches we introduce |(~=)|, which converts complex expressions to strings before checking equality.
...
\todo{Write some decent related work section}

There is the YDS paper, compare and contrast.

There is the work on generic classes in Haskell, i.e. generic instances, drift.
...
\item While our program is written in the typed language Haskell, most of the actual DSL operations are implemented upon a universal data type with dynamic type conversions -- essentially a dynamically typed language. In order to preserve types throughout we could make use of as GADTs \cite{gadts}.
...
Thanks to Stefan O'Rear help with the first version of the \derive{} tool. Thanks to Niklas Broberg for the excellent haskell-src-exts library. Thanks to \todo{Hong Se Yeng} for fruitful discussions on the original instance generation work.
