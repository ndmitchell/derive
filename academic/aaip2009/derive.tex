...
For the current version of \derive{}, we reevaluated the options. Many of the operations were complicated by the introduction of a type, in particular it requires the DSL to track the output type, and propagate type information through the |derive| function. In addition, being tied to Template Haskell was not the advantage we had originally hoped, it's reasonably complex and changes with GHC version.

Derive 1 used Template Haskell, but that sucked. Because of the tool we're writing, we don't have a choice about the output domain. Internally we use haskell-src-exts, but that has a big tree.
\item[Universal abstract syntax tree] The typed one but flattened.
\end{itemize}

We choose universal, but suspect you could acheive the same thing with GADT's and a typed abstract syntax tree. Unfortunately this type information would propagate though everywhere, resulting in lots of complication. Our output language is the language of Haskell declarations. To simplify things we use a universal data type, we translate to and from haskell-src-exts \ref?.



One option would be to generate a string, which can be parsed to form declarations -- however this approach lacks safety,

Concretely, our universal data type is:
