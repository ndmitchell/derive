...
The type we have most freedom over is the DSL, and we present our DSL type in \ref{fig:dsl}. We have divided the DSL in to sections, which we present separately. Each constructor has an interpretation, and a set of circumstances in which it is valid to occur. It would be possible to encode many of the invariants if we used a DSL represented as a GADT \cite{spj:gadt}. However, for simplicity we choose not to.
