...
It would be possible to encode many of the invariants if we used a DSL represented as a GADT \cite{spj:gadt}. However, for simplicity we choose not to.
...
applyEnv dsl env@(Env input ctor field fold) = f dsl
    ...
    f (Application (f -> OList xs)) = g xs
            where  g [x]  = x -- \todo{fold of some sort}
