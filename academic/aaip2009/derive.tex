...
Some of these simplifications could be applied directly to the DSL, for example the removal of |const|. However, other simplifications can't be performed until after |apply|, such as the reduction of |(-)| on \ignore|CtorArity-1|. Currently we do not perform any of these simplifications on the DSL, as they would still need to be applied to the output anyway.
...
It works currently only on a very small range of examples, i.e. Arities, but there is no reason it couldn't be extended to more. There are however some where it couldn't work, such as deriving |isFirst|/|isSecond|/|isThird| functions, as static components are derived from the type.

You need to use some instance extension flags to get the right one and select if multiple ones are in scope. In particular our results need the \ignore|ScopedTypeVariables| extension to be enabled so the same |d_type| is in scope everywhere.

\section{Results}
\label{sec:results}

This section covers the practical results we've got from Derive and our automatic derivation mechanism. We first try and categorise the examples we haven't been able to do, then describe some of the tricks we have developed for specifying more and more examples in our scheme. Next we discuss possible extensions to our scheme, which could increase the power, or make fewer tricks necessary. Finally we cover the timing properties, how fast the derivation takes.

\subsection{Limitations of Automatic Derivation}
\label{sec:failure}

The instance generation scheme given is not complete -- there exist instances whose generator cannot be determined. The Derive tool \cite{derive} is a program for generating instances for user defined data types. Of the 24 instances supported by the Derive tool, 15 are expressed by example, while 9 require manually written instance generators. All the examples which we can't infer related directly to the choice of our |Input| type -- given those restrictions it is impossible to derive them.

\subsubsection{Infix Constructors}

Some types have infix constructors which are dealt with specially, for example:
...
This mainly effects instances which are some type of show, i.e. |Show| and pretty printing. For most instances it doesn't matter, as infix constructors are usually treated exactly equal,
...
Unfortunately our |Input| type does not contain enough information to do this. This restriction limits a handful of  operations, most of which directly manipulate records -- most general definitions do not require information about records. Haskell provides records, which allow fields to be labelled. The |Show| class outputs the field name if present, but the examples have no notion of label-specific behaviour.

\subsection{Type-based definitions}
\label{sec:fromCtorName}

For example, the |Monoid| class requires items of the same type to be processed using |mappend|, but items of a different type use |mempty|. Automatic derivation has no notion of type-specific behaviour. Other examples include |Functor|, |Traversable| and |Uniplate|.

However, the limitation not to create examples with types arises in other contexts. For example, we could write the definition:
...
This function returns the elements contained within a constructor, for example mirroring the operations provided by |fromJust|, and has seen extensive use in the Yhc compiler \cite{yhc}. However, this code generates a warning with GHC that no top-level type signatures have been given. These type signatures can be inferred, but some tools (i.e. Haddock \cite{haddock}) won't include them in documentation without them. However, we can't generate the associated type signatures in our scheme, so this can't be done.
...
instance Binary alpha => Binary (Sample alpha) where
    put x = case x of
        ...
        where putTag = when (length [First{}, Second{}, Third{}] > 1) . putWord8
    ...
    get = do
        ...
        where getTag = if length [First{}, Second{}, Third{}] > 1 then getWord8 else return 0
\end{code}
...
There are two possible reasons for increasing the power -- to make it possible to write new derivations or to make it easier to write existing derivations. The only complex construction that occurs regularly in existing derivations is the explicit arity representation from \S\ref{sec:explicit_arity}. In the the first version of \derive{} it was automatically assumed that the number |3| could be the arity, however this is unsatisfactory -- predictability was lost. One possibility would be to write a meta function |dataArity|, which is automatically translated to the arity -- but this feels unpleasant as now the definitions can't be run as stand alone and we have effectively introduced a new language people need to know (albeit a simple and optional one).

For new derivations the three obvious pieces of information to add are those listed in \S\ref{sec:failure}. Of these three, infix constructors are likely to gain fewest additional instances, so we consider the other two.
...
In most of these cases the only question is whether a type is the type itself, suggesting that might be a sweet spot.
...
Possibly the ugliest encoding is using |length [First{}, Second{}, Third{}]| as the arity.
