...
To generate a guess for the entire output we start by generating guesses for the leaf nodes of the |Output| value, then work upwards combining them. If at any point we see an opportunity to apply one of our custom rules (i.e. |Instance|), we attempt to do so. The important points are how we infer guesses for atoms, how we combine guesses together (always to produce a list), and where we apply our custom rules.

\subsection{Guessing Constant Leafs}

The guess for a string is trivial -- if it has a banned suffix we generate a paramterised, otherwise we just dump the constant string.

The guess for App requires a guess for list, then the string to the app is always constant, and taken from the OApp.

The most complex case is Int, which is guessed as follows:
...
Note that we only guess something as being GuessCtr if it evaluates to constructor 1. The reasons for this are given under the list derivation. We could generate more GuessCtr's but they would be unnecessary.

\subsection{Lists}

The only hard thing to guess are lists. Given a list of guesses we can promote a single |Guess|, combine two adjacent elements with the same guess environment into a single element, or join a complete ordered list of fields or constructors (perhaps in reverse). If by applying these rules we get a single guess, then that guess is valid.
