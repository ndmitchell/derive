...
Using the techniques described in this paper, these relationships can often be automatically inferred from a single example. To define the generation of \textit{all} |NFData| instances, we require an example to be given for the |Sample| data types:
...
Now we turn to the derivation scheme. Given a single result of the |Output| type, for a particular sample |Input|, we wish to derive a suitable DSL. It may not be possible to derive a suitable DSL, so our derivation function must allow for the possibility of failure. Instead of producing at most one DSL, we instead produce a list of DSLs, following the lead of Wadler \citet{wadler:list_of_successes}. Once again, we can implement this in Haskell as:
...
\noindent Values of |Input| for the |Sample| data type (from \S\ref{sec:introduction}) and the |Language| data type (\S\ref{sec:introduction}) are:
...
\ignore\begin{code}
instance (Show alpha, Ord alpha, Show beta, Ord beta) => ShowOrd (Either alpha beta) where ellipses
\end{code}
...
\begin{code}
guess (OList xs) = mapMaybe (liftM fromLists . collapse . toLists) (mapM guess xs)
...
%Currently some redundant brackets are removed by the transformations described in \S\ref{sec:output_simplify}.
